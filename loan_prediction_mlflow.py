...
dataset[numerical_cols] = dataset[numerical_cols].apply(lambda x: x.clip(*x.quantile([0.05, 0.95]))) ## Handling Outliers
...
def eval_metrics(actual, pred): ## Metrics for evaluation of a model
...
def mlflow_logging(model, X, y, name):
    with mlflow.start_run() as run:
        ...
        mlflow.log_params(model.best_params_) # Logging best parameters from gridsearch
        
        mlflow.log_metric("Mean CV score", model.best_score_) ## Log the metrics
        mlflow.log_metric("Accuracy", accuracy)
        mlflow.log_metric("f1-score", f1)
        mlflow.log_metric("AUC", auc)

        mlflow.log_artifact("plots/ROC_curve.png") ## Logging artifacts and model
